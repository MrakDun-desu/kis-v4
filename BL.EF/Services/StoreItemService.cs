...
public class StoreItemService(KisDbContext dbContext, Mapper mapper)
    ...
    public List<StoreItemReadAllModel> ReadAll() {
        return mapper.ToModels(dbContext.StoreItems
            .Where(e => !e.Deleted)
    ...
    public StoreItemReadModel? Read(int id) {
        return mapper.ToModel(dbContext.StoreItems.Find(id));
    }
    ...
    public bool Update(int id, StoreItemUpdateModel updateModel) {
        ...
        if (updateModel.UnitName is not null) {
            entity.UnitName = updateModel.UnitName;
        }
        
        if (updateModel.BarmanCanStock.HasValue) {
            entity.BarmanCanStock = updateModel.BarmanCanStock.Value;
        }
